...
Name the application and pass Deployment Manager the configuration file.
...
In the GCP console window, navigate to the Monitoring screen (Navigation menu > Stackdriver > Monitoring).
Click Log in with Google.
If asked, select your Qwiklabs account.
...
In the Create your free Workspace screen, click the Create workspace button. This will create a new Stackdriver monitoring workspace. A single Stackdriver workspace can contain multiple GCP project and provide a "single pane of glass" to monitor all resources.
On the Add Google Cloud Platform projects to monitor screen, click the Continue button.
Click the Skip AWS Setup button. We will not setup AWS monitoring, but there are Stackdriver monitor agents available for AWS as well.
On the Install the Stackdriver Agents screen, click Continue.
...
59ca9c1e84cea6b5.png

Configure an uptime check and alert policy for the application.
Now that the application is running, you will setup Stackdriver Alerts and some Stackdriver dashboards.

Configure an Uptime Check
On the Stackdriver window or tab, click on Uptime Checks menu, click Uptime Checks Overview. Click on Add Uptime Check.
...
< give the suitable name of Uptime Check >
...
Click Advanced Options and specify the following, leaving the remaining settings at the default values.
Port
...
Click Test. If the test fails, make sure that the service is still working. Also check to see that the firewall rule exists and is correct. If the test succeeds, click Save.

After the Uptime Check is saved, Stackdriver will offer to create an alerting policy.

Configure an Alerting Policy and Notification
Click Create Alert Policy.
e929d9d6b9a89fa3.png

Go to Metric tab, give a display name in the Untitled Condition.

Click Save.

Configure notifications and finish the alerting policy
In Notifications, click the dropdown next to the Notifications Channel Type.
Select Email as the notification channel, and enter a valid email address.
...
< leave blank to allow the default name >
...
Use Apache Bench to apply load to the service.
...
View the results on the Dashboard in Stackdriver.
You can also view the instance groups in Console to see if autoscaling has been triggered. On the Navigation menu, click Compute Engine > Instance groups. Your results may look similar to the image below:
8159c717188bfcb1.png

Simulate a service outage.
Remove the firewall to simulate an outage.
On the Navigation menu, click VPC Network > Firewall rules.
As shown in the image below, select the firewall rule that is allowing TCP 8080 traffic, and click Delete.
70290821269130cf.png

After some time, you should receive a notification email. The notification latency setting determines how long after a policy is triggered before a notification is sent.
